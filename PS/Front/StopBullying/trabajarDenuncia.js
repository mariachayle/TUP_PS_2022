$(document).ready(function () {
    ...
    $.ajax({
        url: "https://localhost:5001/Denuncias/ObtenerDenunciasPendientes",
        type: "GET",
...
function crearTabla(datos) {
    ...
    //  html += "<td>" + datos[i].fecha + "</td>";
        ...
        html += '<td><a type="button" id="btnDenuncia"  data-toggle="tooltip" onclick="getId(' + datos[i].idDenuncia + ')"  class="btn-sm btn-info" >Trabajar</a></th>';
...
function updateDenuncia(idDen, idDirector, idNexo,prioridad, notas) {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.ok) {
                Swal.fire("Denuncia Guardada exitosamente");
                // reload pagina
...
function finDenuncia(idDen,idDirector,idNexo, prioridad, notas) {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.ok) {
                Swal.fire("Denuncia cerrada exitosamente");
                // reload pagina
              setTimeout(function () { location.reload(); }, 1000);
            } else {
                Swal.fire("Problema en el Servidor");
