$(document).ready(function () {
    ...
    $.ajax({
        url: "https://localhost:5001/Denuncias/ObtenerDenunciasEnProgreso",
        type: "GET",
...
function crearTabla(datos) {
    for (var i = 0; i < datos.length; i++) {
        ...
        html += '<td><a type="button" id="btnDenuncia"  data-toggle="tooltip" onclick="getId(' + datos[i].idDenuncia + ')"  class="btn-sm btn-info" >Actualizar</a></th>';
...
function updateDenuncia(idDen, idDirector, idNexo,prioridad, notas) {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.ok) {
                Swal.fire("Denuncia Actualizada exitosamente");
            //    document.getElementById("modalDetalle").setAttribute("hidden",true);
            //    document.getElementById("tablaPrimera").removeAttribute("hidden");
                // reload pagina
